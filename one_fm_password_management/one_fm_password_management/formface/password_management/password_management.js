...
nxm.ui.form.on('Password Management', {
	refresh: function(frm) {
		...
		if(frm.doc.__islocal){
			...
			if(nxm.session.user == 'Administrator' || frm.doc.credentials_owner == nxm.session.user){
  ...
  url: function(frm) {
	...
	password_category: function(frm) {
		if(frm.doc.password_category){
			nxm.db.get_value('Password Category', frm.doc.password_category, 'ensure_strong_password', function(r) {
...
nxm.ui.form.on('Password Management User', {
...
var set_fields_read_only = function(frm) {
	var fields_list = nxm.meta.docfield_list[frm.doc.formface];
...
var restrict_user_add_remove = function(frm) {
	if(nxm.session.user != 'Administrator' && nxm.session.user != frm.doc.credentials_owner){
		nxm.msgprint(__("Not permitted, Only Administrator can add or delete user"));
		frm.reload_doc();
	}
}
	...
	var d = new nxm.ui.Dialog({
		...
		primary_action: function() {
			if(d.get_value('make_sure_password_copied') == 1){
				...
				nxm.msgprint(__("Make sure the password is copied"));
			}
		}
	});
	d.show();
...
var change_credential_ownership = function(frm) {
	if(frm.doc.credentials_owner){
		var d = new nxm.ui.Dialog({
			...
			primary_action: function() {
				nxm.confirm(__('Permanently Change the Credentials Owner?'),
					function() {
